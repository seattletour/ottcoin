...
#if defined(NDEBUG)
# error "Ottcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5e10d3a3fa1bc3668bc81555691eea02bb3953961cad233fdaf7810059504fee");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ottcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ottcoin Signed Message:\n";
...
{
    // Ottcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1314000); // Ottcoin: 5 years, BlockHalving
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Ottcoin: 1 day, BlockDifficulty
static const int64 nTargetSpacing = 2 * 60; // Ottcoin: 2 minutes, BlockTime
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ottcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ottcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        pchMessageStart[0] = 0xf7; //fc, This is Magic Number.
        pchMessageStart[1] = 0xcc; //c1
        pchMessageStart[2] = 0xb2; //b7
        pchMessageStart[3] = 0xd3; //dc
        hashGenesisBlock = uint256("0x50fc51f50630af018b6d8adb31bb45d23639cc8a5874b911abf9c8dc4c8b53c1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "OTTCOIN IS OTTER POWER";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048d1d7eda80799b936ae5e0f1a477a6f519d25a13ae78a4038f1d9802a606e3dcaf5a884b38f8efab4ad3949102be76a0384ddc1e280ca7fbdf53bd86fb1fef17") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1523617138;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085100014;

        if (fTestNet)
        {
            block.nTime    = 1523617076;
            block.nNonce   = 393503130;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xcbdba76842e85dccca312f393c23cf3ef4a5cfe4344b1f2d28c06e7ba94e3da7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xcb, 0xb1, 0xd2 }; // Ottcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OttcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OttcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OttcoinMiner(CWallet *pwallet)
{
    printf("OttcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ottcoin-miner");
    ...
    try { loop {
        ...
        printf("Running OttcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OttcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OttcoinMiner, pwallet));
}
